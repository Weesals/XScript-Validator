...
namespace RTS4.Environment.XScript.RMS {
    public class XRandomMap {
        ...
        public int TileSizeX { get { return simulation.Terrain.Width; } }
        public int TileSizeZ { get { return simulation.Terrain.Height; } }
        public XReal MapSizeX { get { return simulation.Terrain.Width * MetersPerTile; } }
        public XReal MapSizeZ { get { return simulation.Terrain.Height * MetersPerTile; } }
        public readonly XReal MetersPerTile = 2;
        ...
        List<RMObjectDefinition> objectDefinitions = new List<RMObjectDefinition>();
        ...
        public XReal rmRandFloat(XReal min, XReal max) { return min + (max - min) * (XReal)rand.NextDouble(); }
        public int rmRandInt(int min, int max) { return rand.Next(min, max); }

        public XReal rmGetSeaLevel() { return simulation.SeaLevel; }
        public void rmSetSeaLevel(XReal level) { simulation.SeaLevel = level; }
        ...
        public void rmSetMapSize(XReal x, XReal z) {
            simulation.Terrain.Resize((x / MetersPerTile).RoundedToInt, (z / MetersPerTile).RoundedToInt);
        }

        public void rmTerrainInitialize(string baseTerrain, XReal height) {
            var type = Resources.Content.TerrainFromString(baseTerrain);
            /*Debug.Assert(type != null,
                "Unable to find " + baseTerrain + " terrain type");*/
            for(int x=0;x<simulation.Terrain.Width;++x) {
                for(int y=0;y<simulation.Terrain.Height;++y) {
                    simulation.Terrain[x, y] = new TerrainNode() { Height = height, Type = type };
                }
            }
            areaMap = new RMAreaMap(TileSizeX, TileSizeZ, metersToTiles);
        }
        ...
        public XReal rmAreaTilesToFraction(int tiles) {
            return (XReal)tiles / (TileSizeX * TileSizeX);
        }

        private XReal Clamp(XReal v, XReal v1, XReal v2) {
            if (v < v1) return v1; else if (v > v2) return v2; else return v;
        }
        private int Clamp(int v, int v1, int v2) {
            if (v < v1) return v1; else if (v > v2) return v2; else return v;
        }
        public XReal rmXTilesToFraction(int x) { return Clamp((XReal)x / (TileSizeX), 0, 1); }
        public XReal rmZTilesToFraction(int z) { return Clamp((XReal)z / (TileSizeZ), 0, 1); }
        public int rmXFractionToTiles(XReal x) { return Clamp((x * (TileSizeX - 1)).RoundedToInt, 0, TileSizeX - 1); }
        public int rmZFractionToTiles(XReal z) { return Clamp((z * (TileSizeZ - 1)).RoundedToInt, 0, TileSizeZ - 1); }
        public XReal rmXFractionToMeters(XReal x) { XReal m = TileSizeX * MetersPerTile; return Clamp(x * m, 0, m); }
        public XReal rmZFractionToMeters(XReal z) { XReal m = TileSizeZ * MetersPerTile; return Clamp(z * m, 0, m); }
        public XReal rmXMetersToFraction(XReal x) { return Clamp(x / (TileSizeX * MetersPerTile), 0, 1); }
        public XReal rmZMetersToFraction(XReal z) { return Clamp(z / (TileSizeX * MetersPerTile), 0, 1); }
        public XReal rmXTilesToMeters(int x) { return rmXFractionToMeters(rmXTilesToFraction(x)); }
        public XReal rmZTilesToMeters(int z) { return rmZFractionToMeters(rmZTilesToFraction(z)); }
        public int rmXMetersToTiles(XReal x) { return rmXFractionToTiles(rmXMetersToFraction(x)); }
        public int rmZMetersToTiles(XReal z) { return rmZFractionToTiles(rmZMetersToFraction(z)); }
        ...
        public XReal rmDegreesToRadians(XReal degrees) { return degrees * (XReal)Math.PI / 180; }
        public XReal rmRadiansToDegrees(XReal radians) { return radians / 180 * (XReal)Math.PI; }
        ...
        public void rmAddAreaCliffEdgeAvoidClass(int areaID, int avoidID, XReal minDist) { }
        ...
        public void rmAddAreaInfluencePoint(int areaID, XReal xFraction, XReal zFraction) { }
         ...
         */
        public void rmAddAreaInfluenceSegment(int areaID, XReal xFraction1, XReal zFraction1, XReal xFraction2, XReal zFraction2) { }
        ...
        public bool rmBuildArea(int areaID) {
            ...
            if (!area.Position.X.IsNaN) {
                Debug.Assert(area.Position.X >= 0 && area.Position.Y >= 0 &&
                    area.Position.X < TileSizeX * MetersPerTile && area.Position.Y < TileSizeZ * MetersPerTile,
                    "Out of bounds");
            }
            for (int t = 100; t >= 0; --t) {
                XReal tAreaX = area.Position.X, tAreaY = area.Position.Y;
                if (tAreaX.IsNaN) {
                    tAreaX = (XReal)(rand.NextDouble() * (TileSizeX - 1) * MetersPerTile);
                    tAreaY = (XReal)(rand.NextDouble() * (TileSizeZ - 1) * MetersPerTile);
                }
                bool ok = true;
                ...
                if (ok) { area.Position = new XVector2(tAreaX, tAreaY); break; }
                else if (!area.Position.X.IsNaN || t == 0) {
                    if (area.WarnOnFailure) {
                        Console.WriteLine("Unable to place area " + area.Name + " constraints are not satisfied");
                    }
                    return false;
                }
            }
            int areaX = rmXMetersToTiles(area.Position.X),
                areaY = rmZMetersToTiles(area.Position.Y);
            int areaSize = rmAreaFractionToTiles(area.MinSize + (area.MaxSize - area.MinSize) * (XReal)rand.NextDouble());
            List<TerrainType> types = new List<TerrainType>();
            for (int l = 0; l < area.TerrainLayers.Count; ++l) {
                types.Add(Resources.Content.TerrainFromString(area.TerrainLayers[l].TerrainType));
            }
            var influence = new RMAreaCreator(simulation.Terrain.Width, simulation.Terrain.Height, metersToTiles);
            influence.AddArea(new XPoint2(areaX, areaY), areaSize, area.Coherence, delegate(XPoint2 cpnt) {
                for (int c = 0; c < area.Constraints.Count; ++c) {
                    var constraint = area.Constraints[c];
                    if (!constraint.IsPointOk(cpnt.X * MetersPerTile, cpnt.Y * MetersPerTile)) return false;
                }
                return true;
            });
            influence.BlurArea((area.InfluenceBlurDistance / MetersPerTile).ToInt);
            var cliffType = (area.CliffType != null ? Resources.Content.TerrainFromString(area.CliffType + " Cliff") : null);
            area.ActiveArea = XRectangle.Invalid;
            influence.WithAreaTiles(delegate(XPoint2 pnt, int dist) {
                if (dist >= 0) {
                    areaMap.SetTileArea(pnt, area);
                    area.ActiveArea = area.ActiveArea.ExpandToInclude(pnt);
                }
                int x = pnt.X, y = pnt.Y;
                var terrain = simulation.Terrain[x, y];
                XReal heightBlendTiles = area.HeightBlendDistance / MetersPerTile + 1;
                if (!area.BaseHeight.IsNaN) {
                    if (dist < -heightBlendTiles / 2) {
                        // Out of range: Do nothing
                    } else if (dist < heightBlendTiles / 2) {
                        XReal lerp = dist / heightBlendTiles + (XReal)0.5f;
                ...
                if (cliffType != null) {
                    if (dist > -2 && dist < 2) terrain.Type = cliffType;
                }
                simulation.Terrain[x, y] = terrain;
            }, 0, 10);
        ...
        public void rmSetAreaMinBlobDistance(int areaID, XReal dist) { }
        ...
        public void rmSetAreaSize(int areaID, XReal minFraction, XReal maxFraction) {
            areas[areaID].MinSize = minFraction;
                //1.4f * (float)Math.Sqrt(rmAreaFractionToMeters(minFraction) / Math.PI);
            areas[areaID].MaxSize = maxFraction;
                //1.4f * (float)Math.Sqrt(rmAreaFractionToMeters(maxFraction) / Math.PI);
        }
        ...
        public void rmSetAreaWarnFailure(int areaID, bool warn) {
            areas[areaID].WarnOnFailure = warn;
        }

        /**Sets the water type for an area.*/
        public void rmSetAreaWaterType(int areaID, string waterName) {
            areas[areaID].TerrainLayers.Add(new RMArea.TerrainLayer() { TerrainType = waterName.Trim(new [] { ' ' }) });
        }
        ...
        public void rmSetConnectionBaseHeight(int connectionID, XReal width) { }

        /**Sets the base terrain cost for a connection.*/
        public void rmSetConnectionBaseTerrainCost(int connectionID, XReal cost) { }

        /**Sets area coherence (0.0-1.0).*/
        public void rmSetConnectionCoherence(int connectionID, XReal width) { }

        /**Sets how smoothly connection height blends into surroundings.*/
        public void rmSetConnectionHeightBlend(int connectionID, XReal width) { }
         ...
         */
        public void rmSetConnectionPositionVariance(int connectionID, XReal variance) { }

        /**Sets connection edge smoothing distance (distance is number of neighboring points to consider in each direction).*/
        public void rmSetConnectionSmoothDistance(int connectionID, XReal width) { }

        /**Sets the terrain cost for a connection.*/
        public void rmSetConnectionTerrainCost(int connectionID, string terrainTypeName, XReal cost) { }
         ...
         */
        public void rmSetConnectionType(int connectionID, int connectionType, bool connectAll, XReal connectPercentage) { }
        ...
        public void rmSetConnectionWidth(int connectionID, XReal width, XReal variance) { }
        ...
        public int rmCreateTerrainDistanceConstraint(string name, string type, bool passable, XReal distance) { return -1; }

        /**Make a constraint to be close to terrain with certain a passability.*/
        public int rmCreateTerrainMaxDistanceConstraint(string name, string type, bool passable, XReal distance) { return -1; }

        /**Make a type distance constraint.*/
        public int rmCreateTypeDistanceConstraint(string name, string className, XReal distance) { return -1; }


        ////////////// class FairLoc //////////////
        // http://aomcodereference.googlecode.com/svn/trunk/src/aom/scripting/xs/rm/FairLoc.java
        /**Adds some fairLoc placement info.*/
        public int rmAddFairLoc(string unitName, bool forward, bool inside, XReal minPlayerDist, XReal maxPlayerDist, XReal locDist, XReal edgeDist, bool playerArea, bool teamArea) { return -1; }
        ...
        public XReal rmFairLocXFraction(int playerID, int index) { return -1; }

	    /**Gets a player's fairLoc z fraction.*/
        public XReal rmFairLocZFraction(int playerID, int index) { return -1; }
        ...
        public void rmSetObjectDefMaxDistance(int defID, XReal dist) { }

	    /**Set the minimum distance for the object definition (in meters).*/
        public void rmSetObjectDefMinDistance(int defID, XReal dist) { }
        ...
        public int rmCreateObjectDef(string name) {
            objectDefinitions.Add(new RMObjectDefinition(name) {
            });
            return objectDefinitions.Count - 1;
        }
	
	    /**Returns the number of units placed by this objectDefID.*/
        public int rmGetNumberUnitsPlaced(int objectDefID) {
            if (objectDefinitions[objectDefID].PlacedObjects == null) return 0;
            return objectDefinitions[objectDefID].PlacedObjects.Count;
        }
	
	    /**Returns a unit ID that was placed by the objectDefID.*/
        public int rmGetUnitPlaced(int objectDefID, int index) {
            if (objectDefinitions[objectDefID].PlacedObjects == null || objectDefinitions[objectDefID].PlacedObjects.Count == 0) return -1;
            return objectDefinitions[objectDefID].PlacedObjects[index].Id;
        }
        ...
        public int rmGetUnitPlacedOfPlayer(int objectDefID, int playerID) {
            if (objectDefinitions[objectDefID].PlacedObjects == null || objectDefinitions[objectDefID].PlacedObjects.Count == 0) return -1;
            return objectDefinitions[objectDefID].PlacedObjects[0].Id;
        }
        ...
        public void rmPlaceObjectDefAtAreaLoc(int defID, int playerID, int areaID, long placeCount) {
            //return objectDefinitions[defID].PlaceObjectAt(;
        }
	     ...
	     * <li>"favor"</li>
	     */
        public void rmAddPlayerResource(int playerID, string resourceName, XReal amount) { players[playerID].GetResource(resourceName).Amount += amount.ToInt; }
	
	    /**Gets the number of players on the given team*/
        public int rmGetNumberPlayersOnTeam(int teamID) { return players.Count(p => p.Team == teamID); }
        ...
        public string rmGetPlayerName(int playerID) { return players[playerID].Name; }

	    /**Gets the team the specified player is on.*/
        public int rmGetPlayerTeam(int playerID) { return players[playerID].Team; }
        ...
        public void rmSetPlayerLocation(int playerID, XReal xFraction, XReal zFraction) {
            players[playerID].Position = fracToMeters(new XVector2(xFraction, zFraction));
        }

	    /**Gets a player's start location x fraction.*/
        public XReal rmPlayerLocXFraction(int playerID) { return rmXMetersToFraction(players[playerID].Position.X); }

	    /**Gets a player's start location z fraction.*/
        public XReal rmPlayerLocZFraction(int playerID) { return rmXMetersToFraction(players[playerID].Position.Y); }
	     ...
	     */
        public void rmSetPlacementSection(XReal fromPercent, XReal toPercent) { }
	     ...
	     */
        public void rmSetPlayerPlacementArea(XReal minX, XReal minZ, XReal maxX, XReal maxZ) { }
	     ...
	     * <li>"favor"</li>
	     */
        public void rmSetPlayerResource(int playerID, string resourceName, XReal amount) { players[playerID].GetResource(resourceName).Amount = amount.ToInt; }
	     ...
	     */
        public void rmSetTeamSpacingModifier(XReal modifier) { }
        ...
        ////////////// class Trigger //////////////
        // http://aomcodereference.googlecode.com/svn/trunk/doc/aom/scripting/xs/rm/Trigger.html
        ...
        public void rmSetTriggerConditionParamFloat(string paramName, XReal value, bool add) { }
        ...
        public void rmSetTriggerEffectParamFloat(string paramName, XReal value, bool add) { }
        ...
        public int rmTriggerID(string triggerName) { return -1; }

        public XRandomMap(int seed, Simulation _simulation) {
            cMapSize = 1;
